...
namespace PuzzleSolvers.Nanogram.Tests.PerformanceTests {
    public class PuzzleSolvers {
        string outputFolderPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop)+@"\NANOGRAM_SOLVER";
        Dictionary<string, IPuzzle> _puzzles;
        Dictionary<string, List<ISolution>> _solutions;
        public PuzzleSolvers() {
        ...
        [Theory]
        [InlineData("webpbn000001", "#1  Dancer ")]
        [InlineData("webpbn000006", "#6  Cat ")]
        [InlineData("webpbn000021", "#21  Skid ")]
        [InlineData("webpbn000027", "#27  Bucks ")]
        internal void SolveForAny(string puzzleId, string puzzleDescription) {
            ...
            var puzzleSolver = Factory.CreatePuzzleSolver();
            var res = puzzleSolver.SolveForAnySolution(_puzzles[puzzleId], out ISolution solution);

            File.WriteAllLines(file, solution.Grid.GetAsString().Split('\n'));
        ...
        internal void SolveForAll(string puzzleId, string puzzleDescription) {
            ...
            var puzzleSolver = Factory.CreatePuzzleSolver();
            var res = puzzleSolver.SolveForManySolutions(_puzzles[puzzleId], out List<ISolution> solvedSolutions);
        ...
        [InlineData("webpbn000021", "#21  Skid ")]
        [InlineData("webpbn000027", "#27  Bucks ")]
        internal void SolveNoGuess(string puzzleId, string puzzleDescription) {
            var file = outputFolderPath + @$"\{puzzleId}_{puzzleDescription}_resNoGuess.txt";

            var puzzleSolver = Factory.CreatePuzzleSolver();
            var res = puzzleSolver.TrySolvingWithoutGuessing(_puzzles[puzzleId], out ISolution partSolution);


            File.WriteAllLines(file, partSolution.Grid.GetAsString().Split('\n'));
